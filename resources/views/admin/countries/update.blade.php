<x-admin::layouts.dashboard title="Úprava státu">
    <form action="{{ route('admin.countries.update', $country->id) }}" method="post">
        @csrf
        <h1 class="text-center font-bold text-xl">Úprava státu {{ $country->name }}</h1>
        <div class="flex flex-col mb-4 mt-5">
            <div class="flex flex-row gap-5">
                <div class="w-6/12">
                    <x-admin.forms.input type="text" name="name" id="name" label="Název" value="{{ $country->name }}"/>
                </div>
                <div class="w-6/12">
                    <x-admin.forms.input type="text" name="short_name" id="short_name" label="Zkratka" value="{{ $country->short_name }}"/>
                </div>
            </div>
